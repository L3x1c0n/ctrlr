...
**Phase 1 — Prowlarr (Docker)** ✅ DONE 2026-04-22
Prowlarr running in Docker on port 9696. All public indexers migrated. XSpeeds private tracker integrated via two custom Cardigann definitions: `xspeeds-movies.yml` (category=12 → Radarr, tag: movies) and `xspeeds-tv.yml` (category=126 → Sonarr, tag: tv). Native Prowlarr XSpeeds definition was broken (GET without do=search param). XSpeeds global search silently drops movies — split per category group is required. Definitions live at `/home/gh05t/docker/prowlarr/config/Definitions/Custom/`.
