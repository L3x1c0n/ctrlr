          ...
          <div className="overflow-x-auto"><table className="w-full text-xs md:text-sm font-mono table-fixed">
            <thead>
              <tr className="text-[#999] text-xs uppercase border-b border-[#1a1a2e]">
                ...
                <th className="text-left py-1 px-3 hidden md:table-cell w-[80px]">By</th>
                <th className="py-1 pl-3 w-[88px]"></th>
              </tr>
            </thead>
            <tbody>
              {requests.slice(0, 10).map((r, i) => (
                <tr key={r.id} className="border-b border-[#0f0f1a]">
                  ...
                  <td className="text-left px-3 text-[#999] text-xs uppercase hidden md:table-cell">{r.type}</td>
                  <td className={`text-left px-3 whitespace-nowrap ${statusColor[r.status] ?? 'text-[#888]'}`}>
                    {statusLabel[r.status] ?? r.status}
                  </td>
                  <td className="text-left px-3 text-[#999] hidden md:table-cell">{r.requestedBy.displayName}</td>
                  <td className="text-right pl-3 pr-2">
